var im = {
    ...
    reader.addEventListener("loadend", () => { try {
      ...
      CSV.parse(reader.result).forEach(r => {
        ...
        if (r.length != clen) {
          ...
          row.innerHTML = `<td colspan="${clen}">?</td><td>Dòng không hợp lệ</td>`;
        }
          ...
          for (let i=0; i<clen; i++) {
            ...
            if (err==null) {
              if (im.cols[i][2]=="N" && isNaN(r[i])) {
                err = `${im.cols[i][0]} - Số không hợp lệ`;
              }
              else if (im.cols[i][2]==true && r[i]==null) {
                err = `${im.cols[i][0]} - Không thể để trống`;
              }
            }
          }
      ...
      cb.modal("Lỗi mở tệp CSV", err.message)
      console.error(err);
    }});
  ...
  go : first => {
    ...
    // (D3) ALL DONE
      ...
      im.after();
      cb.loading(false);
      cb.modal("Hoàn tất", "Tất cả các mục đã được nhập.");
    }
  }
};